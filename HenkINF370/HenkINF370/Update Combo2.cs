...
{
    public partial class Update_Combo2 : MetroFramework.Forms.MetroForm
    {
        public Update_Combo2()
        {
            InitializeComponent();
        }
        ...
        private void Update_Combo2_Load(object sender, EventArgs e)
        {
            PopulateListBox();
        ...
        private void tctFilter1_TextChanged(object sender, EventArgs e)
        {
        ...
        {
            DialogResult r = MetroFramework.MetroMessageBox.Show(this, "Are you sure you want to Update this Combo?", "Message", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (r == DialogResult.Yes)
            {
                try
                {
                                    ...
                                    while (dr3.Read())
                                    {
                                        ...
                                        if (dr4.HasRows)
                                        {
                                            while (dr4.Read())
                                            {
                                                ...
                                                SqlConnection sqlcon = new SqlConnection(Globals.ConnectionString);
                                                sqlcon.Open();
                                                string cmd = "UPDATE Combos SET ItemName = @ItemName, ComboDescription =@ComboDescription, PizzaBaseID =@PizzaBaseID, DrinkSizeID = @DrinkSizeID, PizzaSizeID = @PizzaSizeID, Price = @Price, ExpireDate = @ExpireDate WHERE ComboDescription ='" + Globals.SelectedComboDescription + "' AND ExpireDate ='" + Globals.SelectedComboExpireDate + "' AND Price ='" + Globals.SelectedComboPrice + "'";
                                                SqlCommand sqlcom = new SqlCommand(cmd, sqlcon);
                    ...
                    MetroFramework.MetroMessageBox.Show(this, "The Combo was Updated Successfully!", "Message", MessageBoxButtons.OK, MessageBoxIcon.None);
